...
class Database {
    ...
    getAllSub() {
        ...
        var x = itr.next();
        while (!x.done) {
            if(!list.includes( x.value.subject.value ) ){
                list.push(x.value.subject.value);
            }
            x = itr.next();
        }
        return list;
    ...
    getAllPred() {
        ...
        var x = itr.next();
        while (!x.done) {
            if(!list.includes( x.value.predicate.value ) ){
                list.push(x.value.predicate.value);
            }
            x = itr.next();
        }
        return list;
    ...
    updateDB() { 
        // console.log("ZACZYNAM", this.database2.size)
        // console.time('someFunction');
        this.database.clear();
        this.database.addAll(this.database2.match(null,null,null))
        // console.timeEnd('someFunction');
        // console.log("KONCZE", this.database2.size)
    }
        ...
        return new Promise((resolve, reject) => {
            let data = (y_quad) => {
                if (y_quad.subject.value === ID || ID === null) {
                    ...
                    if(y_quad.subject.value === "http://metadata.un.org/sdg/SI_POV_DAY1/219/2002"){
                        console.log(y_quad)
                    }
                    // add inverses 
    ...
    async insertRDFCashed(rdf, ID) {
        await this.insertRDFPromise(this.database2, ID, rdf);
    }
